package chessAI;

import java.util.Optional;

import javafx.application.Application;
import javafx.scene.Scene;
import javafx.scene.control.Alert;
import javafx.scene.control.Button;
import javafx.scene.control.ButtonType;
import javafx.scene.control.Alert.AlertType;
import javafx.scene.layout.FlowPane;
import javafx.stage.Stage;

public class Main extends Application {
    
    FlowPane root = new FlowPane();
    Button button = new Button("Play Chess against AI");
    Button buttonVersus = new Button ("Play 2 player chess");
    
    Board board;
    Game game;
    Versus versus = new Versus();
    
    @Override
    public void start(Stage primaryStage) throws Exception {
        // TODO Auto-generated method stub
        
        button.setOnAction((event) -> {
           System.out.println("inside event");
           Alert alert = new Alert(AlertType.CONFIRMATION);
           alert.setTitle("Pick your color");
           alert.setHeaderText("Pick your color");
           alert.setContentText("White or Black?");

           ButtonType buttonTypeOne = new ButtonType("Go first");
           ButtonType buttonTypeTwo = new ButtonType("Go second.");
           
           alert.getButtonTypes().setAll(buttonTypeOne, buttonTypeTwo);
           
           //this shows the alert, without needing "alert.show();"
           Optional<ButtonType> result = alert.showAndWait();
           
           if (result.get() == buttonTypeOne){
               //true = set up board as if player is white
               game = new Game(true);
           } else if (result.get() == buttonTypeTwo) {
               //false = set up board as if player is black
               game = new Game(false);
           } 
           
           board = new Board(primaryStage, game);
           
        });
        
        root.getChildren().addAll(button, buttonVersus);
        
        Scene scene = new Scene(root, 650, 650);
        primaryStage.setTitle("BotGame");
        primaryStage.setScene(scene);
        primaryStage.centerOnScreen();
        primaryStage.show();
        
    }
    
    public static void main (String[] args) {
        launch(args);
    }

}
